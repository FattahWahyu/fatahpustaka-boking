...
include $sroot . "/fatahpustaka-boking/application/third_party/dompdf/autoload.inc.php";
...
public function laporan_anggota()
{
  ...
  $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
  $data['anggota'] = $this->db->get_where('user', ['role_id' => 1])->result_array();
...
{
  $data['anggota'] = $this->ModelUser->cekData(['role_id' => 1])->result_array();
...
public function export_excel_anggota()
{
  $data = [
    ...
    'anggota'  => $this->ModelUser->cekData(['role_id' => 1])->result_array()
  ];
